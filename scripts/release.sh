...
function cross_compile {
  ...
  printf "${blue}Cross-compiling binaries ...${nocolor}\n"
  make clean
  make build-all
}

function upload_binaries {
  printf "${blue}Uploading binaries to GitHub release ${purple}v$release_version${blue} ...${nocolor}\n"
  ...
  for filepath in $binary_prefix-*; do
